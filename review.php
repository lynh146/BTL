...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$restaurant_id) {
        $stmt = $conn->prepare("INSERT INTO restaurants (name, location, price_level) VALUES (?, ?, ?)");
    ...
    // Thêm đánh giá
    $stmt = $conn->prepare("INSERT INTO reviews (restaurant_id, user_id, content, rating) VALUES (?, ?, ?, ?)");
...
</html><!-- viết đánh giá:Bảo
 tạo 1 trang review.php, giao diện trang màu sắc đồng nhất với màu sắc trang chủ.
 cho phép người dùng viết đánh giá về quán ăn
    cần có các trường:
    - Tên quán
    - Địa chỉ quán  
    - Hình ảnh quán (có thể upload từ máy tính)
    - Mức giá (Rẻ, Trung bình, Đắt)
    - Đánh giá sao (1-5 sao)
    - Nội dung đánh giá
    - Tên người đánh giá (có thể lấy từ session nếu đã đăng nhập)
    - Ngày giờ đánh giá (tự động lấy ngày giờ hiện tại)
    - Nút gửi đánh giá
    còn lại bạn tự thêm các trường khác nếu cần thiết
    - khi người dùng gửi đánh giá, sẽ lưu vào database
    - nếu người dùng chưa đăng nhập thì sẽ yêu cầu đăng nhập trước khi gửi đánh giá
    - nếu người dùng đã đăng nhập thì sẽ lấy tên người dùng từ session
